...
export const appWithTranslation = function (WrappedComponent) {
    ...
    class AppWithTranslation extends React.Component<Props> {
        ...
        static async getInitialProps(ctx) {
            ...
            let initialLanguage = ctx.ctx.locale;
            ...
            /* Step 1: Apply initial language from NEXT context */
            if (req && req.i18n) {
            ...
            if (WrappedComponent.getInitialProps) {
                wrappedComponentProps = await WrappedComponent.getInitialProps(
                    ctx
                );
            }
            ...
            if (
                Array.isArray(
                    wrappedComponentProps.pageProps.namespacesRequired
                )
            ) {
            ...
            // We must always send down the defaultNS, otherwise the client will trigger
            // a request for it and issue the "Did not expect server HTML to contain a <h1> in <div>" error
            if (
                ...
                languagesToLoad.forEach((lng) => {
                    ...
                    namespacesRequired.forEach((ns) => {
                        initialI18nStore[lng][ns] =
                            (req.i18n.services.resourceStore.data[lng] || {})[
                                ns
                            ] || {};
                    });
                });
        ...
        render() {
            const { initialLanguage, initialI18nStore, i18nServerInstance } =
                this.props;
